...
// https://github.com/mjebrahimi/EasyCompressor/blob/523bacaae423f7168429d90c3735ae46c0e8a97e/src/EasyCompressor.LZMA/LZMAProperties.cs
public static class LzmaProperties
{
    ...
    public static object[] GetProperties(LZMACompressionLevel compressionLevel, DictionarySize dictionarySize)
    {
        ...
        const bool
            endMarker = false; // Default: false (write End Of Stream marker. By default LZMA doesn't write eos marker, since LZMA decoder knows uncompressed size  stored in .lzma file header.)
        const string
            matchFinder =
                "BT4"; //compressionLevel.GetMatchFinder(); // Default: BT4 (BT2|BT3|BT4|HC4) - depends on compression level
        var numFastBytes =
            (int)compressionLevel; // Default: there is not default but our default is LZMACompressionLevel.UltraFast (5) (from 5 to 273)
    ...
    public static DictionarySize GetDictionarySize(this LZMACompressionLevel compressionLevel)
    {
        return compressionLevel switch
        {
            ...
            _ => throw new InvalidEnumArgumentException(nameof(compressionLevel), Convert.ToInt32(compressionLevel),
                typeof(LZMACompressionLevel))
        };
    }
}
